...
**Brand logo on the index card (optional):** the neighborhoods index cards are frameless (imagery/logos sit over the site topo background — no white card). A neighborhood can render a local brand logo instead of its API photo: drop the SVG in `public/images/neighborhoods/` and add a `slug → path` entry to the `localLogos` map at the top of `index.astro`. Use transparent, single/dark-ink marks (they display over the light `earth-50` background) — masters live in the `JWRG-JWLC-Design` repo. Current logo cards: Aubrie Place, Bragg Farm (1-color), Cannady Mill, Cedar Knolls (nobg mark), Colvard Farms (1-color), Tennyson (1-color dark blue), Woodland Park (dark), Yancey Farms (full dark-green lockup).
