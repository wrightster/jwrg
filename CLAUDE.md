...
3. **Taking one down (JWRG only):** add its slug to `HIDDEN_NEIGHBORHOODS` in `src/lib/api.ts`. The office has no per-site neighborhood publication flag yet (asked for in `OFFICE_MCP_REQUESTS.md`), so the shim filters it out of `fetchNeighborhoods()` (grid, map, sitemap, llms.txt) and makes `fetchNeighborhood()` return null, which redirects the detail and lot routes to `/neighborhoods`. Bragg Farm was hidden this way on 2026-09-14.
