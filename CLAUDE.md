...
**Index card content:** each card is just the logo/photo tile plus a centered footer pair — a single lifecycle pill and one inventory number (no plain-text name, no amenity tags). The pill reads **"Active"** (red, `bg-red-600`) or **"Planned"** (gold, `bg-gold-300` — for office statuses `planning`/`under_development`; everything else, incl. unset, is Active). The number is **available listings** (`listings_count`) when > 0, else **available lots** (from `fetchNeighborhoodLots(slug)`, since `total_lots` is null on the index payload); it self-hides when both are zero. The card link carries `aria-label={name}` so it's labeled in every image branch (logo/photo/placeholder), and logo `<img>`s keep `alt`.
